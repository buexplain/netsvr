...
// TestNewWsConn 测试构造函数
func TestNewWsConn(t *testing.T) {
	// 测试正常 step
	pool := NewWsConn(16)
	if pool == nil {
		t.Fatal("NewWsConn 返回 nil")
	}
	if pool.step != 16 {
...
// TestWsConnGet 测试 Get 方法
func TestWsConnGet(t *testing.T) {
	pool := NewWsConn(16)
...
// TestWsConnPut 测试 Put 方法
func TestWsConnPut(t *testing.T) {
	pool := NewWsConn(16)
...
// TestWsConnPoolReuse 测试对象池复用
func TestWsConnPoolReuse(t *testing.T) {
	pool := NewWsConn(16)
...
// TestWsConnDifferentCapacityLevels 测试不同容量分级
func TestWsConnDifferentCapacityLevels(t *testing.T) {
	pool := NewWsConn(16)
...
// TestWsConnConcurrentAccess 测试并发安全性
func TestWsConnConcurrentAccess(t *testing.T) {
	pool := NewWsConn(16)
...
// TestWsConnMultiplePuts 测试多次 Put
func TestWsConnMultiplePuts(t *testing.T) {
	pool := NewWsConn(16)
...
// BenchmarkWsConnGet 性能基准测试
func BenchmarkWsConnGet(b *testing.B) {
	pool := NewWsConn(16)
...
// BenchmarkWsConnGetParallel 并发性能基准测试
func BenchmarkWsConnGetParallel(b *testing.B) {
	pool := NewWsConn(16)
