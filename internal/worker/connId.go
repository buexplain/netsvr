...
func init() {
	...
	binary.BigEndian.PutUint32(connIdTemplate[0:4], binary.BigEndian.Uint32(ip[12:16]))
	//网关进程的worker服务监听的port
	binary.BigEndian.PutUint16(connIdTemplate[4:6], uint16(port))
}

// New TCP四元组可以保证当前所有连接id不会重复，时间戳可以保证不再同一秒内的连接id不重复
func newConnId(hostPort string) string {
	b := make([]byte, 16)
	copy(b[0:6], connIdTemplate[:])
