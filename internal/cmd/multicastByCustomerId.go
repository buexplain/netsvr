...
// MulticastByCustomerId 根据customerId组播
func MulticastByCustomerId(param []byte, _ *workerManager.ConnProcessor) {
	payload := objPool.MulticastByCustomerId.Get()
	defer objPool.MulticastByCustomerId.Put(payload)
	if err := proto.Unmarshal(param, payload); err != nil {
		log.Logger.Error().Err(err).Msg("Proto unmarshal netsvrProtocol.MulticastByCustomerId failed")
		return
	}
