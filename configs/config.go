...
type config struct {
	...
	Customer struct {
		...
		//最大连接数，超过的会被拒绝
		MaxOnlineNum int
		//该时间段内，客户端连接没有发消息过来，则会超时，连接会被关闭，所以客户端连接必须在该时间间隔内发送心跳字符串
		HeartbeatInterval       time.Duration
		HeartbeatIntervalSecond uint32 `toml:"-"`
		//客户发送数据的大小限制（单位：字节）
	...
	Task struct {
		// 监听的地址，ipv4:port，这个地址必须是内网ipv4地址，外网不允许访问，如果配置的是域名:端口，则会尝试获取域名对应的内网ipv4地址，并打印告警日志
		ListenAddress string
		//读取task连接的超时时间，该时间段内，task连接没有发消息过来，则会超时，连接会被关闭
		ReadDeadline time.Duration
		//发送给task连接的超时时间，该时间段内，没发送成功且有部分写入，则task连接会被关闭
		SendDeadline time.Duration
		//task发送数据的大小限制（单位：字节），task发送了超过该限制的包，则连接会被关闭
		ReceivePackLimit uint32
		//读取task发送数据的缓冲区大小（单位：字节）
		ReadBufferSize int
		//心跳字符串，客户端连接必须定时发送该字符串，用于维持心跳
		HeartbeatMessage BytesConfigItem
	}
...
func init() {
	...
	if Config.Customer.SendMessageType == 0 {
		Config.Customer.SendMessageType = ws.OpText
	} else if Config.Customer.SendMessageType != ws.OpText && Config.Customer.SendMessageType != ws.OpBinary {
		slog.Error(fmt.Sprintf("Config Customer.SendMessageType, must be in [%d,%d]", ws.OpText, ws.OpBinary))
		os.Exit(1)
	}
	if Config.Customer.HeartbeatInterval <= 0 {
		//默认120秒
		Config.Customer.HeartbeatInterval = time.Second * 120
	}
	Config.Customer.HeartbeatIntervalSecond = uint32(Config.Customer.HeartbeatInterval.Seconds())
	...
	if Config.Task.ReadDeadline <= 0 {
		//默认120秒
		Config.Task.ReadDeadline = time.Second * 120
	}
	if Config.Task.SendDeadline <= 0 {
		//默认10秒
		Config.Task.SendDeadline = time.Second * 10
	}
	if Config.Task.ReceivePackLimit == 0 {
		Config.Task.ReceivePackLimit = 1024 * 1024 * 2
	}
	if Config.Task.ReadBufferSize <= 0 {
		Config.Task.ReadBufferSize = 64 * 1024
	}
	if host, port, err := net.SplitHostPort(Config.Task.ListenAddress); err != nil {
		slog.Error("Config Task.ListenAddress, split failed", "error", err)
		os.Exit(1)
	} else {
		if !utils.IsValidIPv4(host) {
			//将域名转为内网地址
			slog.Warn("Config Task.ListenAddress, host is not a valid ipv4 address", "host", host)
			ipv4 := utils.GetHostByName(host)
			if ipv4 == host {
				slog.Warn("Config Task.ListenAddress, convert " + host + " to ipv4 failed")
				os.Exit(1)
			}
			slog.Warn("Config Task.ListenAddress, convert " + host + " to " + ipv4 + " successful")
			Config.Task.ListenAddress = net.JoinHostPort(ipv4, port)
		} else if host == "0.0.0.0" {
			//转为内网地址
			ipv4 := utils.GetLocalIPAddress()
			if ipv4 == "" {
				slog.Error("Config Task.ListenAddress, Not allowed to configure 0.0.0.0")
				os.Exit(1)
			}
			slog.Warn("Config Task.ListenAddress, convert " + host + " to " + ipv4 + " successful")
			Config.Task.ListenAddress = net.JoinHostPort(ipv4, port)
