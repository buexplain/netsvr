...
// RespTopicsConnCount 处理网关发送的主题的连接数
func RespTopicsConnCount(param []byte, processor *connProcessor.ConnProcessor) {
	resp := &respTopicsConnCount.RespTopicsConnCount{}
	...
	//不是客户端请求网关数据，则忽略
	if resp.ReCtx.Cmd != int32(protocol.RouterTopicsConnCount) {
		return
	}
	...
	ret.Data = utils.NewResponse(protocol.RouterTopicsConnCount, map[string]interface{}{"code": 0, "message": "获取网关中主题的连接数成功", "data": resp.Items})
