...
func (uniqId) RequestList(tf *netsvrProtocol.Transfer, _ string) {
	...
	//将结果单播给客户端
	netBus.NetBus.SendToUniqId(tf.UniqId, testUtils.NewResponse(protocol.RouterUniqIdList, map[string]interface{}{"code": 0, "message": "获取网关所有的uniqId成功", "data": resp.Data}))
}
...
func (uniqId) RequestCount(tf *netsvrProtocol.Transfer, _ string) {
	...
	netBus.NetBus.SendToUniqId(tf.UniqId, testUtils.NewResponse(protocol.RouterUniqIdCount, map[string]interface{}{"code": 0, "message": "获取网关中uniqId的数量成功", "data": msg}))
}
